...
export const Settings: React.FC<SettingsProps> = ({ onMenuClick, userName, onToggleTheme, isDark }) => {
  ...
  const handleSaveProfile = async () => {
    try {
      setBioLoading(true); // Re-using loading state
      const { data } = await TechnicianAPI.updateProfile({ name, email });
      if (data.success) {
        setIsEditing(false);
        alert("Profile updated successfully! Some changes may reflect after refresh.");
        // Optionally reload or sync with Auth context if needed
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-800 relative overflow-hidden">
              <div className="flex flex-col items-center">
                <div className="w-20 h-20 rounded-full bg-gray-200 dark:bg-slate-800 animate-pulse mb-4"></div>
                <div className="w-32 h-6 bg-gray-100 dark:bg-slate-800 animate-pulse rounded-md mb-2"></div>
                <div className="w-24 h-4 bg-gray-100 dark:bg-slate-800 animate-pulse rounded-md mb-1"></div>
                <div className="w-40 h-3 bg-gray-100 dark:bg-slate-800 animate-pulse rounded-md mb-4"></div>
                <div className="w-32 h-8 bg-gray-100 dark:bg-slate-800 animate-pulse rounded-full"></div>
              </div>
            </div>

            {/* Preferences Group Skeleton */}
            <div className="space-y-2">
              <div className="w-24 h-3 bg-gray-200 dark:bg-slate-800 animate-pulse rounded-sm mb-2 ml-1"></div>
              <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-800 divide-y divide-gray-50 dark:divide-slate-800">
                {[1, 2].map((i) => (
                  <div key={i} className="p-4 flex items-center justify-between">
                    <div className="flex items-center">
                      <div className="w-10 h-10 bg-gray-100 dark:bg-slate-800 rounded-lg animate-pulse mr-3"></div>
                      <div className="w-24 h-4 bg-gray-50 dark:bg-slate-800 animate-pulse rounded-md"></div>
                    </div>
                    <div className="w-11 h-6 bg-gray-200 dark:bg-slate-800 animate-pulse rounded-full"></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="w-32 h-3 bg-gray-200 dark:bg-slate-800 animate-pulse rounded-sm mb-2 ml-1"></div>
              <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-800 divide-y divide-gray-50 dark:divide-slate-800">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="p-4 flex items-center justify-between">
                    <div className="flex items-center">
                      <div className="w-10 h-10 bg-gray-100 dark:bg-slate-800 rounded-lg animate-pulse mr-3"></div>
                      <div className="space-y-2">
                        <div className="w-32 h-4 bg-gray-50 dark:bg-slate-800 animate-pulse rounded-md"></div>
                        <div className="w-48 h-2 bg-gray-50 dark:bg-slate-800 animate-pulse rounded-md"></div>
                      </div>
                    </div>
                    <div className="w-11 h-6 bg-gray-200 dark:bg-slate-800 animate-pulse rounded-full"></div>
                  </div>
