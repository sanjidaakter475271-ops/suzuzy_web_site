...
const RevenueChart = ({ data }: RevenueChartProps) => {
    ...
    const chartData = React.useMemo(() => {
        ...
        return Object.entries(grouped).map(([name, income]) => ({
            name,
            income,
            expense: income * 0.4 // Placeholder expense ratio for demo
        })).slice(-7); // Last 7 data points
    }, [data]);

    return (
        <div className="bg-white dark:bg-dark-card p-6 rounded-2xl shadow-card border border-surface-border dark:border-dark-border transition-all h-full">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h3 className="text-lg font-bold text-ink-heading dark:text-white">Revenue Overview</h3>
                    <p className="text-xs text-ink-muted">Daily service income trend</p>
                </div>
            ...
            <div className="h-80 w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#8A7B6E' }} />
