...
const QueuedVehiclesWidget = ({ data }: QueuedVehiclesWidgetProps) => {
    const queuedJobs = data || [];
        ...
        <Card className="rounded-[2rem] border border-surface-border dark:border-dark-border shadow-sm hover:shadow-md transition-all h-full">
            <CardContent className="p-6">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <Link href="/service-admin/tickets" className="text-[10px] font-bold text-brand uppercase tracking-widest hover:underline">View All</Link>
                </div>
                {queuedJobs.length === 0 ? (
                        ...
                        {queuedJobs.map(job => (
                            <div key={job.id} className="flex items-center justify-between group cursor-pointer hover:bg-surface-page dark:hover:bg-dark-page p-2 rounded-xl transition-colors">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-xs font-black text-ink-heading dark:text-white">
                                            {job.vehicle_model || job.service_vehicles?.bike_models?.name || 'Unknown Model'}
                                        </p>
                                        <p className="text-[10px] font-bold text-ink-muted uppercase">{job.service_number}</p>
                                    </div>
                                </div>
