...
const CustomerRequestsWidget = ({ data }: CustomerRequestsWidgetProps) => {
                        ...
                        {pendingRequests.map(req => (
                            <div key={req.id} className="bg-surface-page dark:bg-dark-page p-3 rounded-xl border border-surface-border dark:border-dark-border group hover:border-brand/30 transition-colors">
                                <div className="flex justify-between items-start mb-2">
                                    <h4 className="text-xs font-black text-ink-heading dark:text-white uppercase tracking-tight">REQ #{req.id.slice(-6)}</h4>
                                    <span className="text-[10px] text-ink-muted font-mono">{new Date(req.created_at).toLocaleDateString()}</span>
                                </div>
                                <p className="text-xs text-ink-muted line-clamp-2 mb-3 font-medium">{req.service_description}</p>
