'use client';

import React from 'react';
import { Card, CardContent } from '@/components/service-admin/ui';
import { cn } from '@/lib/utils';
import Link from 'next/link';
import { useWorkshopStore } from '@/stores/service-admin/workshopStore';
import { RampData } from '@/hooks/service-admin/useDashboardStats';

interface ActiveRampsWidgetProps {
    apiData?: RampData[];
}

const ActiveRampsWidget: React.FC<ActiveRampsWidgetProps> = ({ apiData }) => {
    // Use API data if available, else fallback to mock store
    const storeRamps = useWorkshopStore((s) => s.ramps);

    // Normalize either source into a common shape
    const ramps = apiData && apiData.length > 0
        ? apiData.map(r => ({
            id: r.id,
            label: `${r.ramp_number}`,
            status: r.status === 'busy' ? 'occupied' as const : 'available' as const,
            title: r.technician_name || `Ramp ${r.ramp_number}`,
            vehicleInfo: r.current_ticket?.vehicle_model || null
        }))
        : storeRamps.slice(0, 10).map(r => ({
            id: r.id,
            label: r.id.replace('R', ''),
            status: r.status,
            title: r.name,
            vehicleInfo: r.vehicleRegNo?.split(' ').pop() || null
        }));

    return (
        <Card className="rounded-[2rem] border border-surface-border dark:border-dark-border shadow-sm hover:shadow-md transition-all">
            <CardContent className="p-6">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-sm font-black uppercase tracking-widest text-ink-heading dark:text-white">Live Bays</h3>
                    <Link href="/service-admin/workshop/ramp-view" className="text-[10px] font-bold text-brand uppercase tracking-widest hover:underline">View All</Link>
                </div>
                <div className="grid grid-cols-5 gap-2">
                    {ramps.map((ramp) => (
                        <div
                            key={ramp.id}
                            className={cn(
                                "aspect-square rounded-xl flex items-center justify-center text-[10px] font-black transition-all",
                                ramp.status === 'available' ? "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20" :
                                    ramp.status === 'occupied' ? "bg-red-500/10 text-red-600 border border-red-500/20 animate-pulse" :
                                        "bg-slate-100 dark:bg-slate-800/50 text-slate-400 dark:text-slate-600 border border-slate-200 dark:border-slate-800"
                            )}
                            title={ramp.title}
                        >
                            {ramp.status === 'occupied' ? (ramp.vehicleInfo || 'BUSY') : ramp.label}
                        </div>
                    ))}
                </div>
                <div className="mt-4 flex gap-4 text-[10px] font-bold text-ink-muted">
                    <span className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-500"></div> Available</span>
                    <span className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-red-500"></div> Busy</span>
                </div>
            </CardContent>
        </Card>
    );
};

export default ActiveRampsWidget;
