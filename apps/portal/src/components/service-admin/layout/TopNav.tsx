        ...
        <header className="sticky top-0 z-30 w-full bg-white dark:bg-dark-card border-b border-surface-border dark:border-dark-border shadow-soft h-16 transition-colors">
            <div className="flex items-center justify-between px-4 h-full">
                ...
                <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="lg:hidden font-bold text-xl text-ink-heading dark:text-white">Suzuky</div>
                ...
                <nav className="hidden lg:flex items-center gap-6 mr-8">
                    {[
                        { label: 'Home', path: '/service-admin/dashboard' },
                        { label: 'Products', path: '/service-admin/inventory/products' },
                        { label: 'Team', path: '/service-admin/workshop/technicians' }
                    ].map((item) => (
                    ...
                    <div className="relative group cursor-pointer">
                        ...
                        <div className="absolute top-full left-0 mt-2 w-48 bg-white dark:bg-dark-card rounded-lg shadow-hover border border-surface-border dark:border-dark-border hidden group-hover:block p-2">
                            <Link href="/service-admin/finance/reports" className="block px-4 py-2 text-sm text-ink-body dark:text-gray-300 hover:bg-surface-hover dark:hover:bg-dark-border rounded-md">Reports</Link>
                            <Link href="/service-admin/settings" className="block px-4 py-2 text-sm text-ink-body dark:text-gray-300 hover:bg-surface-hover dark:hover:bg-dark-border rounded-md">Integrations</Link>
                        </div>
                    </div>
                </nav>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            />
                            <span className="hidden md:block text-xs font-semibold text-ink-heading dark:text-gray-200 mr-1">{user?.name || "Admin"}</span>
                                ...
                                <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-dark-card rounded-xl shadow-hover border border-surface-border dark:border-dark-border z-20 overflow-hidden animate-fade">
                                    ...
                                    <div className="p-4 border-b border-surface-border dark:border-dark-border bg-surface-page/50 dark:bg-dark-page/50">
                                        <div className="flex items-center gap-3">
                                            ...
                                            />
                                            <div>
                                                <p className="text-sm font-bold text-ink-heading dark:text-white">{user?.name || "Admin"}</p>
                                                <p className="text-xs text-ink-muted">{user?.email || "admin@suzuky.com"}</p>
                                            </div>
                                            <span className="ml-auto text-[10px] font-bold bg-brand-soft text-brand px-2 py-0.5 rounded-full">{user?.role || "ADMIN"}</span>
                                        </div>
                                    </div>

                                    {/* Menu Items */}
                                    <div className="p-2">
                                        <Link href="/service-admin/profile" className="w-full flex items-center gap-3 px-3 py-2 text-sm text-ink-body dark:text-gray-300 hover:bg-surface-hover dark:hover:bg-dark-border rounded-lg transition-colors text-left" onClick={() => setProfileOpen(false)}>
                                            <User size={16} /> My Profile
                                        </Link>
                                        <Link href="/service-admin/settings" className="w-full flex items-center gap-3 px-3 py-2 text-sm text-ink-body dark:text-gray-300 hover:bg-surface-hover dark:hover:bg-dark-border rounded-lg transition-colors text-left" onClick={() => setProfileOpen(false)}>
                                            <Settings size={16} /> My Account
                                        </Link>
                                        <Link href="/service-admin/help" className="w-full flex items-center gap-3 px-3 py-2 text-sm text-ink-body dark:text-gray-300 hover:bg-surface-hover dark:hover:bg-dark-border rounded-lg transition-colors text-left" onClick={() => setProfileOpen(false)}>
