...
import { Bike, ChevronDown, ChevronRight, Circle, UserCircle, Settings, HelpCircle } from 'lucide-react';
import { usePathname } from 'next/navigation';
import Link from 'next/link';
import { SIDEBAR_MENU } from '@/constants/service-admin/menu';
            ...
            group.items.forEach(item => {
                if (item.subItems) {
                    const isActiveChild = item.subItems.some(sub =>
                        sub.path === pathname || (pathname === '/service-admin/dashboard' && sub.path === '/service-admin/dashboard')
                    );
            ...
            >
                ...
                <div className="flex items-center gap-3 h-16 px-6 border-b border-surface-border dark:border-dark-border bg-surface-sidebar dark:bg-dark-sidebar flex-shrink-0">
                    <div className="p-1.5 bg-brand rounded-lg text-white shadow-lg">
                        <Bike size={20} fill="currentColor" strokeWidth={2.5} />
                    </div>
                    <span className="text-xl font-bold text-ink-heading dark:text-white tracking-tight">Suzuky</span>
                </div>
                        ...
                        <div key={groupIndex}>
                            ...
                            <ul className="space-y-1">
                                {group.items.map((item, itemIndex) => {
                                    const Icon = item.icon || Bike;
                                    const isActive = !item.subItems && (pathname === item.path || (pathname === '/service-admin/dashboard' && item.path === '/service-admin/dashboard'));
