...
const QueuePage = () => {
    ...
    const queueAppointments = appointments.filter((a: Appointment) => ['pending', 'scheduled'].includes(a.status));

    // Dynamic Stats
    const totalBookings = appointments.length;
    const checkedInCount = appointments.filter((a: Appointment) => a.status === 'scheduled').length;
    ...
    const nextToken = appointments.length > 0
        ? Math.max(...appointments.map(a => a.token || 0)) + 1
        : 1;

    return (
        <div className="p-6 lg:p-8 space-y-8 animate-fade">
            <Breadcrumb items={[{ label: 'Appointments', href: '/service-admin/appointments' }, { label: "Today's Queue" }]} />
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="bg-surface-card dark:bg-dark-card border border-surface-border dark:border-dark-border px-4 py-2 rounded-xl text-center">
                        ...
                        <p className="text-2xl font-black mt-0.5 text-ink-heading dark:text-white">{nextToken}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {isLoading ? (
                        <Card>
                            <CardContent className="p-12 text-center text-ink-muted">Loading queue...</CardContent>
                        </Card>
                    ) : queueAppointments.length > 0 ? (
                        queueAppointments.map((apt: Appointment) => (
                            <Card key={apt.id} className="hover-golden group cursor-pointer transition-all duration-300">
                                <CardContent className="p-4 flex flex-col md:flex-row items-center gap-6">
                                    ...
                                    <div className="flex-1 space-y-1 text-center md:text-left">
                                        <div className="flex flex-col md:flex-row md:items-center gap-2">
                                            <h3 className="font-bold text-lg text-ink-heading dark:text-white">{apt.customerName}</h3>
                                            <span className="hidden md:inline text-ink-border">•</span>
                                            <span className="text-sm font-medium text-ink-muted">{apt.vehicleRegNo} ({apt.vehicleModel})</span>
                                        </div>
                                        <div className="flex items-center justify-center md:justify-start gap-4 text-xs text-ink-muted">
                                            <div className="flex items-center gap-1.5">
                                                <Clock size={14} className={apt.status === 'scheduled' ? 'text-brand' : ''} />
                                                {apt.time || '--:--'}
                                            </div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            ...
                                            className="p-2 hover:bg-blue-50 text-blue-500 rounded-lg transition-colors opacity-80 hover:opacity-100"
                ...
                <div className="space-y-6">
                    <Card className="bg-brand text-white shadow-xl shadow-brand/10">
                        <CardContent className="p-6 text-center space-y-4">
                            <h3 className="font-bold uppercase tracking-widest text-sm text-white/80">Estimated Wait Time</h3>
                            <div className="text-5xl font-black">{Math.max(5, queueAppointments.length * 15)} <span className="text-lg font-bold opacity-80">min</span></div>
