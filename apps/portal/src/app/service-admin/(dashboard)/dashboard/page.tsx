...
import { Calendar, Bike, Loader2, AlertCircle, Package, ArrowUpRight, TrendingDown } from 'lucide-react';
...
// Comprehensive Loading skeleton for dashboard
function DashboardSkeletons() {
    return (
        <div className="space-y-8 animate-pulse">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="bg-white dark:bg-dark-card p-6 rounded-3xl shadow-card border border-surface-border dark:border-dark-border h-32" />
                ))}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[1, 2, 3].map((i) => (
                    <div key={i} className="h-[350px] bg-white dark:bg-dark-card rounded-[2.5rem] border border-surface-border dark:border-white/5" />
                ))}
            </div>
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <div className="xl:col-span-2 h-[400px] bg-white dark:bg-dark-card rounded-[2.5rem] border border-surface-border dark:border-white/5" />
                <div className="h-[400px] bg-white dark:bg-dark-card rounded-[2.5rem] border border-surface-border dark:border-white/5" />
            </div>
        </div>
    );
}
...
export default function DashboardPage() {
    ...
    return (
        <div className="p-6 lg:p-8 space-y-8 animate-fade">
            ...
            {/* Dashboard Content with Skeletons */}
            {isLoading ? (
                <DashboardSkeletons />
            ) : !data ? (
                        ...
                        <div className="relative z-10 flex flex-col lg:flex-row gap-12 items-center">
                            ...
                            <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="bg-white/10 backdrop-blur-xl rounded-[2rem] p-8 border border-white/10 flex flex-col items-center justify-center text-center space-y-2">
                                    ...
                                    <p className="text-4xl font-black">{data?.workshopPulse?.activeJobs ?? 0}</p>
                                ...
                                <div className="bg-white/10 backdrop-blur-xl rounded-[2rem] p-8 border border-white/10 flex flex-col items-center justify-center text-center space-y-2">
                                    ...
                                    <p className="text-4xl font-black">{data?.workshopPulse?.rampUsage ?? '0/0'}</p>
                                ...
                                <div className="bg-white/10 backdrop-blur-xl rounded-[2rem] p-8 border border-white/10 flex flex-col items-center justify-center text-center space-y-2">
                                    ...
                                    <p className="text-4xl font-black">{data?.workshopPulse?.avgTatMinutes ? `${data.workshopPulse.avgTatMinutes}m` : '--'}</p>
