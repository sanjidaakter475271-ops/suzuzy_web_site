...
import StatCounter from '@/components/service-admin/ui/StatCounter';
...
interface Activity {
    title: string;
    time: string;
    iconType: 'check' | 'edit' | 'globe' | 'trending' | 'download';
    color: string;
    category: string;
}
...
const ProfileClient: React.FC<ProfileClientProps> = ({
    ...
    const handleSave = () => {
        setIsEditing(false);
        // In a real app, API call would happen here to update profiles
                        ...
                        <div className="px-6 pb-8 -mt-12 flex flex-col items-center text-center relative z-10">
                            ...
                            <div className="mt-6 w-full grid grid-cols-2 gap-4 border-t border-surface-border dark:border-dark-border pt-6">
                                <div>
                                    ...
                                    <p className="text-sm font-black text-ink-heading dark:text-white">{profileData.joinedDate}</p>
                                </div>
                                <div>
                            ...
                            <div className="flex items-center justify-between group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-ink-muted font-bold">Recently • 103.44.12.11</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            {warehouses.map((wh, i) => (
                                <div key={i} className="flex items-center gap-3 p-3 bg-surface-page dark:bg-dark-page rounded-xl border border-surface-border dark:border-dark-border">
                                    ...
                                    <span className="text-xs font-bold text-ink-heading dark:text-white">{wh.name}</span>
                                    {wh.isPrimary && <div className="ml-auto px-2 py-0.5 bg-success/10 text-success text-[8px] font-black uppercase rounded">Primary</div>}
                                </div>
                            ))}
                        </div>
                    </div>
                                ...
                                <div key={field.key} className="space-y-2 group">
                                    ...
                                    {isEditing ? (
                                        ...
                                        <p className="text-base font-bold text-ink-heading dark:text-white pb-2 border-b-2 border-surface-border dark:border-dark-border group-hover:border-brand/40 transition-colors">
                                            {profileData[field.key as keyof typeof profileData] || 'Not Set'}
                                        </p>
                                    )}
                                </div>
                            ))}
                    ...
                    <div className="bg-surface-card dark:bg-dark-card border border-surface-border dark:border-dark-border rounded-[32px] p-8 shadow-card">
                        ...
                        <div className="space-y-0 relative">
                            ...
                            {activities.map((act, i) => {
                                const Icon = iconMap[act.iconType];
                                return (
